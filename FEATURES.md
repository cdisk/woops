...
**最后更新：** 2026-09-18（CentOS 6/SysV + 排除安装进程误判 live；`woops-agent install`/`uninstall` 子命令成为主安装路径，控制台一行命令改为 curl 二进制；Windows 重装/换 Gateway 热修；资产备注；GitHub 镜像；Demo 自愈；真证书可免 pin；内网 IP/监控索引/英文 locale 等既有项）
...
| `[x]` | docker-compose 全栈 | **拉镜像**：根目录 [`docker-compose.yml`](docker-compose.yml) → Hub `cdisk/woops-{console,control-api,gateway}`（默认标签 `WOOPS_IMAGE_TAG=0.1.11`）；README §A 快速启动；**`tls-init`**：无 `deploy/tls/gateway.*` 时按 PUBLIC URL 自签，并刷新 [`deploy/compose-pin.env`](deploy/compose-pin.env)（SPKI，供 control-api/gateway）。**源码构建**：[`deploy/docker-compose.yml`](deploy/docker-compose.yml)。`deploy/env.prod.example`；服务器 `/opt/ops`（**10.255.17.30**）；profiles `full`+`desktop`；**Gateway `network_mode: host`**；TLS 挂 `deploy/tls`（gitignore）；`Dockerfile.control-api` 用阿里云 Maven + BuildKit `/root/.m2` 缓存。full 另映射 control-api `:9100`、guacd `:4822`（公网靠防火墙）。**「重新部署」**：见 `.cursor/rules/redeploy.mdc`（17.30 源码构建 + Hub 推送 `$ver`/`latest` + 升 compose 默认标签） |
| `[x]` | 公开 Demo 环境 | [`deploy/demo/`](deploy/demo/)：`bootstrap-host.sh`（Docker / socat / acme.sh）、`make-env.sh`（随机密钥 + 真证书 `.env`）、`issue-cert.sh`（acme.sh standalone 签到 `deploy/tls/gateway.*`，`--reloadcmd` 续期后重启 console/gateway）、`docker-compose.demo.yml`（`!override []` 撤掉根 compose 发布的 5432/9100/4822；起容器化 Agent 当演示资产，身份文件落具名卷、容器文件系统一次性）、`Dockerfile.agent` + `entrypoint-agent.sh`（Agent 二进制取自 gateway 镜像；不写 pin，走系统 CA）、`firewall.sh`（只放 22/80/443/9200，docker0/br+ 放行 9201（否则 /ws 超时）；DOCKER-USER 丢弃 Agent 子网 FORWARD 掐断出网）、`reset.sh` + `install-cron.sh`（每小时整点重置）。域名 `woops-demo.tool4dev.net`。分组树与安装码抽到 [`demo-tree.sh`](deploy/demo/demo-tree.sh)，`seed-assets.sh` 与 `reset.sh` 共用：名字**一律英文**（分组名是数据、不走 i18n），且**一个分组签一个安装码**——都用同一个码的话，任何重新注册的 Agent 都会落进该码指向的分组，原分组空掉又被清理删除，分组树会一小时一小时地烂掉；建树必须在清理**之后**（清理会删掉当时没有资产挂着的分组）|
...
| `[x]` | Console 中英 i18n | `vue-i18n`（`legacy:false`）；仅 `en`/`zh`；**默认英文**，`navigator.language` 以 `zh` 开头则中文；目录 `apps/console/src/i18n/{en,zh,index}.js`；`App.vue` 用 `el-config-provider` 同步 Element Plus locale；SFC `useI18n()`，纯 JS `import { t } from '…/i18n'`；控制台可见文案已迁入目录（含 Layout/Login/资产/用户/审计/会话/文件/桌面/端口映射/监控等）。`index.html` 的 `lang` 默认 `en`、由 `main.js` 按 `detectLocale()` 改写（写死 `zh-CN` 会让英文界面被浏览器当中文页并弹「翻译此页」）。**整篇文档类文案一语言一文件**：个人中心 API 说明为 `features/profile/apiTokenDoc{,.en}.md`，`ApiDocPanel` 按 locale 选（两份的标题层级与 `{{API_REQUEST_PREFIX}}` 占位符须保持一致，否则 TOC/请求前缀会错）。**库里的数据不受 i18n 管**，分两种处理：**系统元数据走 i18n 覆盖**——监控项显示名查 `monitorItems.<itemId 点换下划线>`（点会被当嵌套路径），命中则用，否则回落 API 的 `name`（用户自建监控项保留其自起的名），helper 见 `features/monitor/monitorItemLabel.js`，调用点为监控图标题/图例、预警规则的监控项下拉、已忽略预警；**用户数据不覆盖**——预警规则名会被用户改、且被拼进预警标签（`AlertIssueLogic` 拼成 `规则名=值`），故只把 `MetricsBootstrap` 的 4 条默认规则种成英文，老库里的中文默认规则需手工改名。`MetricsBootstrap` 只插不更新（`existsById`/`count()>0`），所以改常量只对全新安装生效。另一类 i18n 管不到的是后端异常原文透传的 toast |
| `[x]` | Console 视觉（冷静工程风） | 全局 `styles/{tokens,element-theme,base}.css`；主色 `#2F5D9F`、浅色底；字体 **IBM Plex Sans/Mono** 经 `@fontsource` **同源自托管**（无 Google Fonts CDN）；登录品牌首屏；Layout 侧栏图标+**顶栏显示当前页标题**（路由 `meta.titleKey`，页内不再重复大标题）；侧栏底部 **版本号**（`package.json` → `v0.1.11`）+ **源码链接**（`https://gitee.com/cdisk/woops`）；业务页筛选/操作留在内容区工具条；会话页顶栏抛光（桌面页保持暗色功能面）；**favicon** `public/favicon.svg`（主）+ `.ico` / apple-touch PNG；登录与侧栏品牌点同源 SVG |
...
| `[x]` | `GET /i/{code}/install.sh` | **过渡期保留**：旧版 `curl … \| bash` 仍可用；新安装走下行 Agent 二进制 `install` 子命令 |
| `[x]` | `GET /i/{code}/install.ps1` / `install.bat` | **过渡期保留**：旧版脚本安装仍可用；新安装走 `woops-agent install`（见下） |
| `[x]` | `GET /i/{code}/agent/{os}/{arch}` | 产物名 `woops-agent-{os}-{arch}`（`.exe`/`.gz`）；`-ldflags=-s -w -X main.Version=…`；**Content-Encoding: gzip** 协商（`curl --compressed` 透明解压）+ 旁路 `?format=gz`；**Docker Gateway** 同步编 `linux/amd64` + `linux/arm64` + `windows/amd64`；gateway/Linux/woopsctl 用当前 Go，**Windows Agent 独立固定 Go 1.20.14**（Go 1.21+ 产物不能运行于 Win7 / Server 2012） |
...
| `[x]` | `woops-agent install` / `uninstall` | **主安装路径**（`go/internal/agent/install`）：控制台一行命令先 `curl`（带 pin + `--compressed`）下载 Agent 二进制，再 `install -gateway -code -pin`。强制刷新顶层 `gateway`/`gatewayTlsSpkiSha256`，保留 `gatewayProxy`/`proxy`/`proxyBridge`（行级合并，测 BOM/CRLF/缩进子键）；原子写 `install-code`；落地二进制并保留一代 `.old`；注册 **systemd** 或 **SysV**（`/etc/init.d/woops-agent` + chkconfig，兼容 CentOS 6）/ Windows Service（`svc/mgr`）；脱离父进程后 `-phase=restart`；冷装等注册最多 60s；**升级失败自动回滚** `.old`。`AgentLive` **排除当前安装进程 PID**（否则 `/tmp/woops-agent install` 被 `pgrep -x` 误判 live，二进制落到 `/opt` 而 StartService 去启 `/usr/local/bin`）。Linux 安装命令探测 curl 是否支持 `--pinnedpubkey`（CentOS 6 curl 7.19 无此选项则回退 `-k` 并告警）。`uninstall`（可选 `-purge`）停删服务与二进制。旧 `install.{sh,ps1,bat}` 过渡期仍嵌入 Gateway |
| `[x]` | install 脚本行为（遗留） | 过渡期仍服务；行为与上列 `woops-agent install` 对齐连接项刷新语义。PowerShell `File.Replace` 备份参数须 `[NullString]::Value`；legacy BAT 三态 `CFG_MODE`。新部署请用 `woops-agent install` |
...
| `[x]` | 安装命令带 proxy URL / 读环境变量写入配置 | 安装：`curl`/`curl.exe` 认 `https_proxy`/`http_proxy`/`ALL_PROXY`；`woops-agent install` 写入 `agent.yaml` `gatewayProxy`；控制台弹窗说明单引号与密码百分号编码 |
| `[x]` | Windows 安装 | 一行命令下载 `woops-agent.exe` 后 `install`（管理员）；OS build &lt; 17763 自动下 WinPTY；注册 Service `woops-agent`；过渡期仍保留 `install.ps1`/`install.bat` |
